...
const PROPS = [
  { name: "value",       required: true,  type: "string",                      defaultVal: "—",           desc: "Currently selected value." },
  { name: "onChange",    required: true,  type: "(value: string) => void",      defaultVal: "—",           desc: "Called with the selected value when an option is chosen." },
  { name: "options",                      type: "string[] | {value, label}[]",  defaultVal: "[]",          desc: "Options list — plain strings or value/label objects." },
  { name: "placeholder",                  type: "string",                       defaultVal: "'Choose…'",   desc: "Placeholder text shown when no value is selected." },
  { name: "label",                        type: "string",                       defaultVal: "undefined",   desc: "Label shown above the combobox." },
  { name: "hasError",                     type: "boolean",                      defaultVal: "false",       desc: "Error state — applies red border." },
  { name: "disabled",                     type: "boolean",                      defaultVal: "false",       desc: "Disables the combobox." },
];
  ...
  return (
    <div style={{ padding: "24px 32px", background: "var(--neutral-00)", minHeight: "100vh" }}>
      ...
      <p style={{ fontSize: 13, color: "var(--text-light)", marginBottom: 0 }}>
        Searchable dropdown. Type to filter options. Extracted from the ObSelect pattern in AddThirdParty.
      </p>
      ...
      <div style={{ background: "var(--neutral-25)", padding: "16px 20px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "264px 264px", gap: "12px 16px" }}>
          <Combobox label="Country" value={value} onChange={(v) => setValue(v)} options={COUNTRIES} placeholder="Type to search…" />
          <Combobox label="Error state" value={errorValue} onChange={(v) => setErrorValue(v)} options={COUNTRIES} placeholder="Required" hasError={errorValue === ""} />
